...
(function() {
  ...
  fs.readdirSync(path).forEach(function (filename) {
    ...
    stack, config, module, mountPoint;
    ...
    config = JSON.parse(fs.readFileSync(Path.join(modulePath, 'package.json')), 'utf8').config;
    mountPoint = Path.join('/', config && config.path ? config.path : filename);
...
nabe
  .use(function(req, res, next){
    renderer.render('page.404.html', res, {
      error: '404 Not found', 
