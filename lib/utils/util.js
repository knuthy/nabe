...
module.exports = {
  ...
  parseDate: function(date, format, culture) {
    ...
    // returns a localized date, depending on format and culture provided
    // console.log('test culture', !jqg.cultures[culture]);
    if(culture !== 'en') {
        // if culture provided is not globalize's default, make sure to load missing culture
        require('globalize/lib/cultures/globalize.culture.' + culture);
    }
    
    return jqg.format(new Date(), format, culture);
  },
